...
pub async fn set_welcome(pool: &Pool, chat_id: i64, text: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET welcome_text = ? WHERE chat_id = ?")
        .bind(text)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn toggle_welcome(pool: &Pool, chat_id: i64, enabled: bool) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET welcome_enabled = ? WHERE chat_id = ?")
        .bind(enabled)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn set_goodbye(pool: &Pool, chat_id: i64, text: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET goodbye_text = ? WHERE chat_id = ?")
        .bind(text)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn toggle_goodbye(pool: &Pool, chat_id: i64, enabled: bool) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET goodbye_enabled = ? WHERE chat_id = ?")
        .bind(enabled)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn set_rules(pool: &Pool, chat_id: i64, rules: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET rules = ? WHERE chat_id = ?")
        .bind(rules)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn add_warn(
    ...
    sqlx::query(
        "INSERT INTO warns (chat_id, user_id, reason, warned_by) VALUES (?, ?, ?, ?)",
    )
    .bind(chat_id)
    .bind(user_id)
    .bind(reason)
...
pub async fn get_warns(pool: &Pool, chat_id: i64, user_id: i64) -> Result<Vec<Warn>, sqlx::Error> {
    sqlx::query_as::<_, Warn>("SELECT * FROM warns WHERE chat_id = ? AND user_id = ? ORDER BY created_at DESC")
        .bind(chat_id)
        .bind(user_id)
        .fetch_all(pool)
        .await
...
pub async fn get_warns_all(pool: &Pool, chat_id: i64) -> Result<Vec<Warn>, sqlx::Error> {
    sqlx::query_as::<_, Warn>("SELECT * FROM warns WHERE chat_id = ? ORDER BY created_at DESC")
        .bind(chat_id)
        .fetch_all(pool)
        .await
}
...
pub async fn reset_warns(pool: &Pool, chat_id: i64, user_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query("DELETE FROM warns WHERE chat_id = ? AND user_id = ?")
        .bind(chat_id)
        .bind(user_id)
        .execute(pool)
        .await?;
...
pub async fn remove_last_warn(pool: &Pool, chat_id: i64, user_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query(
        "DELETE FROM warns WHERE id = (SELECT id FROM warns WHERE chat_id = ? AND user_id = ? ORDER BY created_at DESC LIMIT 1)",
    )
    .bind(chat_id)
    .bind(user_id)
    .execute(pool)
...
pub async fn set_warn_limit(pool: &Pool, chat_id: i64, limit: i32) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET warn_limit = ? WHERE chat_id = ?")
        .bind(limit)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn set_warn_mode(pool: &Pool, chat_id: i64, mode: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET warn_mode = ? WHERE chat_id = ?")
        .bind(mode)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn get_note(pool: &Pool, chat_id: i64, name: &str) -> Result<Option<Note>, sqlx::Error> {
    sqlx::query_as::<_, Note>("SELECT * FROM notes WHERE chat_id = ? AND name = ?")
        .bind(chat_id)
        .bind(name.to_lowercase())
        .fetch_optional(pool)
        .await
...
pub async fn get_all_notes(pool: &Pool, chat_id: i64) -> Result<Vec<Note>, sqlx::Error> {
    sqlx::query_as::<_, Note>("SELECT * FROM notes WHERE chat_id = ? ORDER BY name")
        .bind(chat_id)
        .fetch_all(pool)
        .await
}
...
pub async fn delete_note(pool: &Pool, chat_id: i64, name: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM notes WHERE chat_id = ? AND name = ?")
        .bind(chat_id)
        .bind(name.to_lowercase())
        .execute(pool)
        .await?;
...
pub async fn get_filters(pool: &Pool, chat_id: i64) -> Result<Vec<Filter>, sqlx::Error> {
    sqlx::query_as::<_, Filter>("SELECT * FROM filters WHERE chat_id = ? ORDER BY keyword")
        .bind(chat_id)
        .fetch_all(pool)
        .await
}
...
pub async fn get_filter_by_keyword(
    ...
    sqlx::query_as::<_, Filter>("SELECT * FROM filters WHERE chat_id = ? AND keyword = ?")
        .bind(chat_id)
        .bind(keyword.to_lowercase())
        .fetch_optional(pool)
        .await
...
pub async fn delete_filter(pool: &Pool, chat_id: i64, keyword: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM filters WHERE chat_id = ? AND keyword = ?")
        .bind(chat_id)
        .bind(keyword.to_lowercase())
        .execute(pool)
        .await?;
...
pub async fn delete_all_filters(pool: &Pool, chat_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query("DELETE FROM filters WHERE chat_id = ?")
        .bind(chat_id)
        .execute(pool)
        .await?;
    Ok(())
...
pub async fn add_blacklist(
    ...
    sqlx::query(
        "INSERT INTO blacklist (chat_id, trigger_word, mode) VALUES (?, ?, ?)
...
pub async fn get_blacklist(pool: &Pool, chat_id: i64) -> Result<Vec<Blacklist>, sqlx::Error> {
    sqlx::query_as::<_, Blacklist>(
        "SELECT * FROM blacklist WHERE chat_id = ? ORDER BY trigger_word",
    )
    .bind(chat_id)
    .fetch_all(pool)
    .await
...
pub async fn remove_blacklist(
    ...
    let result = sqlx::query("DELETE FROM blacklist WHERE chat_id = ? AND trigger_word = ?")
        .bind(chat_id)
        .bind(trigger.to_lowercase())
        .execute(pool)
        .await?;
...
pub async fn set_blacklist_mode(
    ...
    sqlx::query("UPDATE blacklist SET mode = ? WHERE chat_id = ?")
        .bind(mode)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn set_antiflood(pool: &Pool, chat_id: i64, count: i32) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET antiflood_count = ? WHERE chat_id = ?")
        .bind(count)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn set_antiflood_mode(pool: &Pool, chat_id: i64, mode: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET antiflood_mode = ? WHERE chat_id = ?")
        .bind(mode)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn enable_command(pool: &Pool, chat_id: i64, command: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM disabled_commands WHERE chat_id = ? AND command = ?")
        .bind(chat_id)
        .bind(command.to_lowercase())
        .execute(pool)
        .await?;
...
pub async fn get_disabled_commands(pool: &Pool, chat_id: i64) -> Result<Vec<String>, sqlx::Error> {
    let rows: Vec<DisabledCommand> = sqlx::query_as(
        "SELECT * FROM disabled_commands WHERE chat_id = ?",
    )
    .bind(chat_id)
    .fetch_all(pool)
    .await?;
...
pub async fn is_command_disabled(pool: &Pool, chat_id: i64, command: &str) -> Result<bool, sqlx::Error> {
    let result: Option<DisabledCommand> = sqlx::query_as(
        "SELECT * FROM disabled_commands WHERE chat_id = ? AND command = ?",
    )
    .bind(chat_id)
    .bind(command.to_lowercase())
    .fetch_optional(pool)
...
pub async fn set_language(pool: &Pool, chat_id: i64, lang: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET language = ? WHERE chat_id = ?")
        .bind(lang)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn set_log_channel(pool: &Pool, chat_id: i64, channel_id: Option<i64>) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE chats SET log_channel = ? WHERE chat_id = ?")
        .bind(channel_id)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn remove_lock(pool: &Pool, chat_id: i64, lock_type: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM locks WHERE chat_id = ? AND lock_type = ?")
        .bind(chat_id)
        .bind(lock_type.to_lowercase())
        .execute(pool)
        .await?;
...
pub async fn get_locks(pool: &Pool, chat_id: i64) -> Result<Vec<String>, sqlx::Error> {
    let rows: Vec<Lock> = sqlx::query_as("SELECT * FROM locks WHERE chat_id = ?")
        .bind(chat_id)
        .fetch_all(pool)
        .await?;
    Ok(rows.into_iter().map(|r| r.lock_type).collect())
}
...
pub async fn remove_all_locks(pool: &Pool, chat_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query("DELETE FROM locks WHERE chat_id = ?")
        .bind(chat_id)
        .execute(pool)
        .await?;
    Ok(())
...
pub async fn gban_user(pool: &Pool, user_id: i64, reason: &str, banned_by: i64) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO gbans (user_id, reason, banned_by) VALUES (?, ?, ?)
...
pub async fn ungban_user(pool: &Pool, user_id: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM gbans WHERE user_id = ?")
        .bind(user_id)
        .execute(pool)
        .await?;
    Ok(result.rows_affected() > 0)
...
pub async fn is_gbanned(pool: &Pool, user_id: i64) -> Result<bool, sqlx::Error> {
    let result: Option<Gban> = sqlx::query_as("SELECT * FROM gbans WHERE user_id = ?")
        .bind(user_id)
        .fetch_optional(pool)
        .await?;
    Ok(result.is_some())
...
pub async fn get_gban(pool: &Pool, user_id: i64) -> Result<Option<Gban>, sqlx::Error> {
    sqlx::query_as::<_, Gban>("SELECT * FROM gbans WHERE user_id = ?")
        .bind(user_id)
        .fetch_optional(pool)
        .await
}
...
pub async fn disconnect_chat(pool: &Pool, user_id: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM connections WHERE user_id = ?")
        .bind(user_id)
        .execute(pool)
        .await?;
    Ok(result.rows_affected() > 0)
...
pub async fn set_report_setting(pool: &Pool, chat_id: i64, enabled: bool) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO report_settings (chat_id, enabled) VALUES (?, ?)
...
pub async fn set_afk(pool: &Pool, user_id: i64, reason: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO afk_users (user_id, reason, is_afk) VALUES (?, ?, 1)
         ON CONFLICT(user_id) DO UPDATE SET reason = excluded.reason, is_afk = 1",
    )
    .bind(user_id)
    .bind(reason)
    .execute(pool)
...
pub async fn remove_afk(pool: &Pool, user_id: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM afk_users WHERE user_id = ?")
        .bind(user_id)
        .execute(pool)
        .await?;
    Ok(result.rows_affected() > 0)
...
pub async fn is_afk(pool: &Pool, user_id: i64) -> Result<bool, sqlx::Error> {
    let row: Option<AfkUser> = sqlx::query_as("SELECT * FROM afk_users WHERE user_id = ? AND is_afk = 1")
        .bind(user_id)
        .fetch_optional(pool)
        .await?;
    Ok(row.is_some())
...
pub async fn get_afk(pool: &Pool, user_id: i64) -> Result<Option<AfkUser>, sqlx::Error> {
    sqlx::query_as::<_, AfkUser>("SELECT * FROM afk_users WHERE user_id = ? AND is_afk = 1")
        .bind(user_id)
        .fetch_optional(pool)
        .await
}
...
pub async fn add_blacklist_sticker(pool: &Pool, chat_id: i64, sticker_set: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT OR IGNORE INTO blacklist_stickers (chat_id, sticker_set) VALUES (?, ?)",
    )
    .bind(chat_id)
...
pub async fn remove_blacklist_sticker(pool: &Pool, chat_id: i64, sticker_set: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM blacklist_stickers WHERE chat_id = ? AND sticker_set = ?")
        .bind(chat_id)
        .bind(sticker_set.to_lowercase())
        .execute(pool)
        .await?;
...
pub async fn get_blacklist_stickers(pool: &Pool, chat_id: i64) -> Result<Vec<String>, sqlx::Error> {
    let rows: Vec<BlacklistSticker> = sqlx::query_as(
        "SELECT * FROM blacklist_stickers WHERE chat_id = ?",
    )
    .bind(chat_id)
    .fetch_all(pool)
    .await?;
...
pub async fn set_blsticker_mode(pool: &Pool, chat_id: i64, mode: i32) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO blsticker_settings (chat_id, mode) VALUES (?, ?)
...
pub async fn get_blsticker_mode(pool: &Pool, chat_id: i64) -> Result<i32, sqlx::Error> {
    let row: Option<BlstickerSetting> = sqlx::query_as(
        "SELECT * FROM blsticker_settings WHERE chat_id = ?",
    )
    .bind(chat_id)
    .fetch_optional(pool)
    .await?;
...
pub async fn get_user_chats_count(pool: &Pool, user_id: i64) -> Result<i64, sqlx::Error> {
    let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM user_chats WHERE user_id = ?")
        .bind(user_id)
        .fetch_one(pool)
        .await?;
    Ok(count.0)
...
pub async fn get_captcha_settings(pool: &Pool, chat_id: i64) -> Result<CaptchaSettings, sqlx::Error> {
    let row: Option<CaptchaSettings> = sqlx::query_as(
        "SELECT * FROM captcha_settings WHERE chat_id = ?",
    )
    .bind(chat_id)
    .fetch_optional(pool)
    .await?;
...
pub async fn set_captcha_enabled(pool: &Pool, chat_id: i64, enabled: bool) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO captcha_settings (chat_id, enabled) VALUES (?, ?)
         ON CONFLICT(chat_id) DO UPDATE SET enabled = excluded.enabled",
    )
    .bind(chat_id)
    .bind(enabled)
...
pub async fn set_captcha_mode(pool: &Pool, chat_id: i64, mode: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO captcha_settings (chat_id, captcha_mode) VALUES (?, ?)
...
pub async fn set_captcha_timeout(pool: &Pool, chat_id: i64, timeout: i64) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO captcha_settings (chat_id, timeout_min) VALUES (?, ?)
...
pub async fn set_captcha_action(pool: &Pool, chat_id: i64, action: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO captcha_settings (chat_id, failure_action) VALUES (?, ?)
...
pub async fn set_captcha_max_attempts(pool: &Pool, chat_id: i64, max: i64) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO captcha_settings (chat_id, max_attempts) VALUES (?, ?)
...
pub async fn create_captcha_attempt(
    ...
    sqlx::query(
        "INSERT INTO captcha_attempts (user_id, chat_id, answer, attempts, message_id, expires_at) VALUES (?, ?, ?, 0, ?, ?)
...
pub async fn get_captcha_attempt(pool: &Pool, user_id: i64, chat_id: i64) -> Result<Option<CaptchaAttempt>, sqlx::Error> {
    sqlx::query_as::<_, CaptchaAttempt>(
        "SELECT * FROM captcha_attempts WHERE user_id = ? AND chat_id = ?",
    )
    .bind(user_id)
    .bind(chat_id)
    .fetch_optional(pool)
...
pub async fn increment_captcha_attempts(pool: &Pool, user_id: i64, chat_id: i64) -> Result<i64, sqlx::Error> {
    sqlx::query(
        "UPDATE captcha_attempts SET attempts = attempts + 1 WHERE user_id = ? AND chat_id = ?",
    )
    .bind(user_id)
    .bind(chat_id)
    .execute(pool)
    .await?;
    let row: Option<CaptchaAttempt> = sqlx::query_as(
        "SELECT * FROM captcha_attempts WHERE user_id = ? AND chat_id = ?",
    )
    .bind(user_id)
    .bind(chat_id)
    .fetch_optional(pool)
    .await?;
    Ok(row.map(|r| r.attempts).unwrap_or(0))
}

pub async fn delete_captcha_attempt(pool: &Pool, user_id: i64, chat_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query("DELETE FROM captcha_attempts WHERE user_id = ? AND chat_id = ?")
        .bind(user_id)
        .bind(chat_id)
        .execute(pool)
        .await?;
...
pub async fn delete_all_captcha_attempts(pool: &Pool, chat_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query("DELETE FROM captcha_attempts WHERE chat_id = ?")
        .bind(chat_id)
        .execute(pool)
        .await?;
    Ok(())
...
pub async fn add_dev_team(pool: &Pool, user_id: i64, role: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO dev_team (user_id, role) VALUES (?, ?)
...
pub async fn remove_dev_team(pool: &Pool, user_id: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM dev_team WHERE user_id = ?")
        .bind(user_id)
        .execute(pool)
        .await?;
    Ok(result.rows_affected() > 0)
...
pub async fn get_dev_team_member(pool: &Pool, user_id: i64) -> Result<Option<DevTeamMember>, sqlx::Error> {
    sqlx::query_as::<_, DevTeamMember>("SELECT * FROM dev_team WHERE user_id = ?")
        .bind(user_id)
        .fetch_optional(pool)
        .await
}
...
pub async fn is_dev_team(pool: &Pool, user_id: i64) -> Result<bool, sqlx::Error> {
    let row: Option<DevTeamMember> = sqlx::query_as(
        "SELECT * FROM dev_team WHERE user_id = ?",
    )
    .bind(user_id)
    .fetch_optional(pool)
    .await?;
...
pub async fn create_federation(pool: &Pool, fed_id: &str, fed_name: &str, owner_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO federations (fed_id, fed_name, owner_id) VALUES (?, ?, ?)",
    )
    .bind(fed_id)
    .bind(fed_name)
    .bind(owner_id)
...
pub async fn delete_federation(pool: &Pool, fed_id: &str) -> Result<(), sqlx::Error> {
    ...
    sqlx::query("DELETE FROM fed_bans WHERE fed_id = ?").bind(fed_id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM fed_admins WHERE fed_id = ?").bind(fed_id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM fed_chats WHERE fed_id = ?").bind(fed_id).execute(&mut *tx).await?;
    sqlx::query("DELETE FROM federations WHERE fed_id = ?").bind(fed_id).execute(&mut *tx).await?;
    tx.commit().await?;
    Ok(())
}

pub async fn get_federation(pool: &Pool, fed_id: &str) -> Result<Option<Federation>, sqlx::Error> {
    sqlx::query_as::<_, Federation>("SELECT * FROM federations WHERE fed_id = ?")
        .bind(fed_id)
        .fetch_optional(pool)
        .await
}
...
pub async fn get_fed_by_owner(pool: &Pool, owner_id: i64) -> Result<Option<Federation>, sqlx::Error> {
    sqlx::query_as::<_, Federation>("SELECT * FROM federations WHERE owner_id = ?")
        .bind(owner_id)
        .fetch_optional(pool)
        .await
}
...
pub async fn leave_federation(pool: &Pool, chat_id: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM fed_chats WHERE chat_id = ?")
        .bind(chat_id)
        .execute(pool)
        .await?;
    Ok(result.rows_affected() > 0)
...
pub async fn get_fed_chat(pool: &Pool, chat_id: i64) -> Result<Option<FedChat>, sqlx::Error> {
    sqlx::query_as::<_, FedChat>("SELECT * FROM fed_chats WHERE chat_id = ?")
        .bind(chat_id)
        .fetch_optional(pool)
        .await
}
...
pub async fn get_fed_chats(pool: &Pool, fed_id: &str) -> Result<Vec<FedChat>, sqlx::Error> {
    sqlx::query_as::<_, FedChat>("SELECT * FROM fed_chats WHERE fed_id = ?")
        .bind(fed_id)
        .fetch_all(pool)
        .await
}
...
pub async fn remove_fed_admin(pool: &Pool, fed_id: &str, user_id: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM fed_admins WHERE fed_id = ? AND user_id = ?")
        .bind(fed_id)
        .bind(user_id)
        .execute(pool)
        .await?;
...
pub async fn get_fed_admins(pool: &Pool, fed_id: &str) -> Result<Vec<FedAdmin>, sqlx::Error> {
    sqlx::query_as::<_, FedAdmin>("SELECT * FROM fed_admins WHERE fed_id = ?")
        .bind(fed_id)
        .fetch_all(pool)
        .await
}
...
pub async fn is_fed_admin(pool: &Pool, fed_id: &str, user_id: i64) -> Result<bool, sqlx::Error> {
    let row: Option<FedAdmin> = sqlx::query_as(
        "SELECT * FROM fed_admins WHERE fed_id = ? AND user_id = ?",
    )
    .bind(fed_id)
    .bind(user_id)
    .fetch_optional(pool)
...
pub async fn fban_user(pool: &Pool, fed_id: &str, user_id: i64, user_name: &str, reason: &str) -> Result<(), sqlx::Error> {
    ...
    sqlx::query(
        "INSERT INTO fed_bans (fed_id, user_id, user_name, reason, banned_at) VALUES (?, ?, ?, ?, ?)
...
pub async fn unfban_user(pool: &Pool, fed_id: &str, user_id: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM fed_bans WHERE fed_id = ? AND user_id = ?")
        .bind(fed_id)
        .bind(user_id)
        .execute(pool)
        .await?;
...
pub async fn get_fed_bans(pool: &Pool, fed_id: &str) -> Result<Vec<FedBan>, sqlx::Error> {
    sqlx::query_as::<_, FedBan>("SELECT * FROM fed_bans WHERE fed_id = ? ORDER BY banned_at DESC")
        .bind(fed_id)
        .fetch_all(pool)
        .await
}
...
pub async fn get_user_fban_all(pool: &Pool, user_id: i64) -> Result<Vec<FedBan>, sqlx::Error> {
    sqlx::query_as::<_, FedBan>("SELECT * FROM fed_bans WHERE user_id = ?")
        .bind(user_id)
        .fetch_all(pool)
        .await
}
...
pub async fn set_fed_rules(pool: &Pool, fed_id: &str, rules: &str) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE federations SET fed_rules = ? WHERE fed_id = ?")
        .bind(rules)
        .bind(fed_id)
        .execute(pool)
        .await?;
...
pub async fn set_fed_log(pool: &Pool, fed_id: &str, log_channel: Option<i64>) -> Result<(), sqlx::Error> {
    sqlx::query("UPDATE federations SET fed_log = ? WHERE fed_id = ?")
        .bind(log_channel)
        .bind(fed_id)
        .execute(pool)
        .await?;
...
pub async fn get_user_info(pool: &Pool, user_id: i64) -> Result<Option<UserInfoRow>, sqlx::Error> {
    sqlx::query_as::<_, UserInfoRow>("SELECT * FROM user_info WHERE user_id = ?")
        .bind(user_id)
        .fetch_optional(pool)
        .await
}
...
pub async fn set_user_bio(pool: &Pool, user_id: i64, bio: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO user_info (user_id, bio) VALUES (?, ?)
...
pub async fn set_user_me(pool: &Pool, user_id: i64, me_info: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO user_info (user_id, me_info) VALUES (?, ?)
...
pub async fn get_cleaner_settings(pool: &Pool, chat_id: i64) -> Result<CleanerSettings, sqlx::Error> {
    let row: Option<CleanerSettings> = sqlx::query_as(
        "SELECT * FROM cleaner_settings WHERE chat_id = ?",
    )
    .bind(chat_id)
    .fetch_optional(pool)
    .await?;
...
pub async fn set_clean_service(pool: &Pool, chat_id: i64, enabled: bool) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO cleaner_settings (chat_id, clean_service) VALUES (?, ?)
...
pub async fn set_clean_bluetext(pool: &Pool, chat_id: i64, enabled: bool) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO cleaner_settings (chat_id, clean_bluetext) VALUES (?, ?)
...
pub async fn add_reaction(pool: &Pool, chat_id: i64, keyword: &str, emoji: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "INSERT INTO reactions (chat_id, keyword, emoji) VALUES (?, ?, ?)
...
pub async fn remove_reaction(pool: &Pool, chat_id: i64, keyword: &str) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM reactions WHERE chat_id = ? AND keyword = ?")
        .bind(chat_id)
        .bind(keyword)
        .execute(pool)
        .await?;
...
pub async fn get_reactions(pool: &Pool, chat_id: i64) -> Result<Vec<Reaction>, sqlx::Error> {
    sqlx::query_as::<_, Reaction>("SELECT * FROM reactions WHERE chat_id = ?")
        .bind(chat_id)
        .fetch_all(pool)
        .await
}
